...
const GetAllKala = (props)=> {
    const {homepartId, id} = useParams();
    const [allKala, getAllKala] = useState([]);
    ...
    const reNewGetAllKala=()=>{
      axios.get('https://starfoods.ir/api/listKalaOfPictureApi',{
        ...
        .then((data) => {
          getAllKala(data.data.kala.map((element,index)=>
                        ...
                        parseInt(element.requested) === 0 ? 
                         ...
                         : 
                            <span className="prikalaGroupPricece fw-bold mt-1 float-start" id={"norequest" + element.GoodSn}>
                                <button value="1" id={"afterButton" + element.GoodSn}
          ...
          let modalItems=[];
            ...
            modalItems.push(
                <button data-bs-dismiss="modal" className="btn btn-sm btn-danger buyButton"
                   onClick={(e)=>buySomething(data.data.amountExist,data.data.freeExistance,data.data.zeroExistance,data.data.costLimit,data.data.costError,data.data.amountUnit*index,data.data.kalaId,data.data.defaultUnit,e,event)}>
                  {index+' '+data.data.secondUnit+' معادل '+' '+index*data.data.amountUnit+' '+data.data.defaultUnit}
                </button>
              )}
