...
export default function Message() {
    ...
    const addNewMessage=()=>{
        ...
        axios.get("https://s.starfoods.ir/api/doAddMessage",{params:{
            pmContent:newMessage,
            psn:localStorage.getItem("psn")
        }}).then((data)=>{
            if(data.data==="good"){
                axios.get("https://s.starfoods.ir/api/messageList",{params:{psn:localStorage.getItem("psn")}}).then((data)=>{setMessages(data.data.messages.map((message)=> 
                <>
                    <br/>
