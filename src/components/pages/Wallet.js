...
export default function Wallet() {
    ...
    const fillAnswers=()=>{
        axios.get("http://192.168.10.27:8080/api/addMoneyToCase",{params:{
            answer1:document.querySelector("#exampleFormControlTextarea1").value,
            answer2:document.querySelector("#exampleFormControlTextarea2").value,
            answer3:document.querySelector("#exampleFormControlTextarea3").value,
            nazarId:document.querySelector("#nazarId").value
    }
        }).then((data)=>{
            setYesNo(false)
        })
                    ...
                    {questions ? <div className="row rounded-3 mt-3" id="questionPart">
                        <div div className="col-lg-12 p-2" >
                            <ul className="list-group  pe-1">
                                ...
                                <li className="list-group-item question">
                                    <div className="mb-3">
                                        <label for="exampleFormControlTextarea2" className="form-label"> <b> سوال دوم : {secondQuestions}</b></label>
                                        <textarea className="form-control" name="answer2" required id="exampleFormControlTextarea2" minlength="15" rows="3"></textarea>
                                    </div>
                                </li>
                                <li className="list-group-item question">
                                    <div className="mb-3">
                                        <label for="exampleFormControlTextarea3" className="form-label"> <b> سوال سوم: {thirdQuestions}</b></label>
                                        <textarea className="form-control" name="answer3" required id="exampleFormControlTextarea3" minlength="15" rows="3"></textarea>
                                    </div>
                                </li>
                                <span className="list-group-item question textn-end">
                                    <input type="hidden" name="takhfif" value="" />
                                    <button className="walletbutton" onClick={()=>fillAnswers()} type="button"> ارسال </button>
                                </span>
                            </ul>
                        </div >
                    </div > : ""
