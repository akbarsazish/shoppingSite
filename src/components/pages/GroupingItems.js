...
export default function GroupingItems(props) {
    ...
    //
    useEffect(() => {
        fetch("http://192.168.10.24:8080/api/getSubGroupList/?mainGrId=" + id)
    ...
    useEffect(() => {
        ...
        const requestProduct=(psn,goodSn,event)=>{
            axios.get("http://192.168.10.24:8080/api/addRequestedProduct",{params:{
              customerId:psn,
              productId:goodSn
            }}).then((data)=>{
                renewGroupItems();
          ...
          const cancelRequestKala=(psn,goodSn,event)=>{
            axios.get("http://192.168.10.24:8080/api/cancelRequestedProduct",{params:{
              psn:psn,
              gsn:goodSn
            }}).then((data)=>{
                renewGroupItems();
    ...
    const renewGroupItems=()=>{
        axios.get("http://192.168.10.24:8080/api/getMainGroupKala",{params:{
                        ...
                        {element.Amount>0?
                            ...
                            (element.Amount>0 || element.activePishKharid>0 || element.freeExistance>0)? ''
                             :( element.requested===0?
                                ...
                                <button value="0" id={"preButton"+element.GoodSn} onClick={(event)=>requestProduct(3609,element.GoodSn,event)}   className="btn btn-sm btn-danger selectAmount">خبرم کنید <FontAwesomeIcon icon={faBell}></FontAwesomeIcon></button></span>
                                 :
    ...
    const showBuyModal=(goodSn,event)=>{
        ...
        axios.get("http://192.168.10.24:8080/api/getUnitsForUpdate",{params:{
            Pcode:goodSn,
            psn:localStorage.getItem("psn")
        }})
        .then((data) => {
